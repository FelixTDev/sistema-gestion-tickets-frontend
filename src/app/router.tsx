...
function ProtectedRoute({ roles, children }: { roles: Role[]; children: ReactNode }) { const { session, isLoading } = useAuth(); if (isLoading) return <div className="state" role="status">Validando sesión…</div>; return !session ? <Navigate to="/login" replace /> : hasRole(session, roles) ? children : <AccessDenied /> }
